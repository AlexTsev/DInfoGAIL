...
class ClonosEncoder(object):
    ...
    def create_policy(observation_dimensions,latent_dimensions, action_dimensions):
        with tf.name_scope('ClonosEncoder'):
            x = Input(shape=(observation_dimensions+latent_dimensions,), dtype=tf.float64)

            l1 = Dense(100, activation='tanh')(x)
            l2 = Dense(100, activation='tanh')(l1)

            out = Dense(action_dimensions)(l2)

        model = Model(inputs=x, outputs=out)

        return model, x, l1, l2
...
class TRPOAgent(object):
    ...
    def __init__(self, env, observation_dimensions=11, action_dimensions=3):
        """
        ...
        self.sess = tf.Session(config=config)
        self.model, self.x, self.h, self.h1 = Policy.create_policy(self.observation_dimensions, self.action_dimensions)
        #self.model, self.x, self.l1, self.l2 = ClonosEncoder.create_policy(self.observation_dimensions, self.action_dimensions)
        ...
        self.actions_ph = tf.placeholder(tf.float64, shape=(None,
                                                action_dimensions),
                                         )
    ...
    def rollout(self, mini_batch):
        ...
        while not_enough_samples:
            ...
            for t in range(self.path_size):
                ...
                if mini_batch % 100 == 0:
                    ...
                    obs_ep = observation + [episode]
                    # print('obs_ep2:', obs_ep)
        ...
        if mini_batch % 100 == 0:
            np.savetxt(str(myconfig['exp'])+'_'+str(mini_batch)+"_observation_log.csv", np.asarray(log_observations)
                       , delimiter=',', header='var1,var2,var3,var4,var5,var6,var7,var8,var9,var10,var11,episode',
                       comments='')
    ...
    def train(self, expert_observations, expert_actions):
        ...
        saver = tf.train.Saver(self.model.weights)
        saver.restore(self.sess, "./checkpoint/bcloning.ckpt")
        #saver = tf.train.import_meta_graph('./VAE/results_gumbel_softmax/checkpoint/2021-01-26-16-53-11/encoder/encoder_model9999.ckpt.meta')
        #saver.restore(self.sess, "./VAE/results_gumbel_softmax/checkpoint/2021-01-26-16-53-11/encoder/encoder_model9999.ckpt")
