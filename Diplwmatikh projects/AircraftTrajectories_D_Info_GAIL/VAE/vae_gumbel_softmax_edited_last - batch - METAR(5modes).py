...
from environment import environment_metar
...
from utils import conjugate_gradient, set_from_flat, kl, self_kl, \
    ...
    unnormalize_action, unnormalize_observation, unnormalize_observation_metar
...
flags.DEFINE_string('checkpoint_dir', os.getcwd() + '/results_gumbel_softmax/checkpoint/' + 'run8-enriched_Metar(5modes)', 'Directory for checkpoints')#current_time
...
flags.DEFINE_integer('encoder_n_input', 15, 'input size')
...
#flags.DEFINE_integer('latent_nz', 5, 'latent size')
flags.DEFINE_integer('decoder_n_input', 15, 'input size')
...
def apply_normalization_observations(df):
    ...
    df_norm_obs = pd.DataFrame(np_scaled, columns=['longitude', 'latitude', 'altitude', 'timestamp', 'Pressure_surface', 'Relative_humidity_isobaric', 'Temperature_isobaric', 'Wind_speed_gust_surface', 'u-component_of_wind_isobaric', 'v-component_of_wind_isobaric', 'drct', 'sknt', 'alti', 'vsby', 'gust'])
    return df_norm_obs
...
def pandas1(obs):
    df_norm_obs = pd.DataFrame(obs, columns=['longitude', 'latitude', 'altitude', 'timestamp', 'Pressure_surface', 'Relative_humidity_isobaric', 'Temperature_isobaric', 'Wind_speed_gust_surface', 'u-component_of_wind_isobaric', 'v-component_of_wind_isobaric', 'drct', 'sknt', 'alti', 'vsby', 'gust'])
    return df_norm_obs
...
def starting_points_2(observations,fname):
    ...
    for i in range(4):
        pd.DataFrame(observations_prcnt[i], columns=['trajectory_ID', 'longitude', 'latitude',
                                                     ...
                                                     'drct', 'sknt', 'alti', 'vsby', 'gust']).to_csv(
...
def encoder(x, latent, temperature):
    ...
    while(i<32):
        ...
        encoder_input_layer = Input(shape=(20,), batch_size=1, tensor=enc_inp, dtype=tf.float64)
...
def decoder(x, latents):
    ...
    #Tensorflow Keras Layers
    decoder_input_layer = Input(shape=(20,), batch_size=32, tensor=dec_inp, dtype=tf.float64)
...
print('test_trajectories:', test_flight_num)
...
print('train_trajectories:', train_flight_num)
...
def train():
    ...
    inputs = tf.placeholder(tf.float64, shape=[None, 15], name='enc_inp')
...
def renameVAE():
    ...
    OLD_CHECKPOINT_FILE = './results_gumbel_softmax/checkpoint/run8-enriched_Metar(5modes)' + '/encoder/encoder2000/encoder_model_e1999-i327040.ckpt'  # 327040 #'/encoder/encoder_model49599.ckpt'
    NEW_CHECKPOINT_FILE = './results_gumbel_softmax/checkpoint/run8-enriched_Metar(5modes)' + '/test/encoder/encoder_model_e2000.ckpt'  # '/encoder_new(2)/encoder_model_new49600.ckpt'
    ...
    OLD_CHECKPOINT_FILE2 = './results_gumbel_softmax/checkpoint/run8-enriched_Metar(5modes)' + '/decoder/decoder2000/decoder_model_e1999-i327040.ckpt'  # 1999-49999.ckpt'
    NEW_CHECKPOINT_FILE2 = './results_gumbel_softmax/checkpoint/run8-enriched_Metar(5modes)' + '/test/decoder/decoder_model_e2000.ckpt'  # 2000_new50000.ckpt'
...
def rename():
    ...
    # RENAME MAP LAYERS-BIASES-GRAPHS
    OLD_CHECKPOINT_FILE = './results_gumbel_softmax/checkpoint/run8-enriched_Metar(5modes)' + '/encoder/encoder2000/encoder_model_e1999-i327040.ckpt'  # '/encoder/encoder_model49599.ckpt'
    NEW_CHECKPOINT_FILE = './results_gumbel_softmax/checkpoint/run8-enriched_Metar(5modes)' + '/trpo_plugins/encoder/encoder_model_e2000.ckpt'  # '/encoder_new(2)/encoder_model_new49600.ckpt'
    # OLD_CHECKPOINT_FILE = FLAGS.checkpoint_dir + '/model9999.ckpt'
    # NEW_CHECKPOINT_FILE = FLAGS.checkpoint_dir + '/model_new/model_new9999.ckpt'
    ...
    OLD_CHECKPOINT_FILE2 = './results_gumbel_softmax/checkpoint/run8-enriched_Metar(5modes)' + '/decoder/decoder2000/decoder_model_e1999-i327040.ckpt'  # 1999-49999.ckpt'
    NEW_CHECKPOINT_FILE2 = './results_gumbel_softmax/checkpoint/run8-enriched_Metar(5modes)' + '/trpo_plugins/decoder/decoder_model_e2000.ckpt'  # 2000_new50000.ckpt'
...
class Agent(object):
    def __init__(self, observation_dimensions=15, latent_dimensions=5, action_dimensions=3):#latent_dimensions=5
        ...
        self.env = environment_metar.Environment()
    ...
    def one_hot_encoding(self, x):
        ...
        encoded = to_categorical(argmax, num_classes=5)#3)
        # print('enc:', encoded)
    ...
    def test(self):
        out_file = 'results_gumbel_softmax/checkpoint/run8-enriched_Metar(5modes)/test/0_VAE_results_metar.csv'
        action_out_file = 'results_gumbel_softmax/checkpoint/run8-enriched_Metar(5modes)/test/0_actions_VAE_results_metar.csv'

        enc_saver = tf.train.Saver(self.model2.weights)
        enc_saver.restore(self.sess, "results_gumbel_softmax/checkpoint/run8-enriched_Metar(5modes)/test/encoder/encoder_model_e2000.ckpt")#run2(batch-32-modes3)
        saver = tf.train.Saver(self.model.weights)
        saver.restore(self.sess, "results_gumbel_softmax/checkpoint/run8-enriched_Metar(5modes)/test/decoder/decoder_model_e2000.ckpt")#run2(batch-32-modes3)
        ...
        with open(out_file, 'w') as results, open(action_out_file, 'w') as action_results:
            ...
            for i_episode in range(50):
                ...
                for t in range(1000):
                    ...
                    _, observation2, reward, done = self.env.step(action, t)
