...
class Discriminator(object):
    ...
    def __init__(self, observation_dimensions=11, action_dimensions=3):
        ...
        self.expert_samples_observations = Input(shape=(observation_dimensions,), dtype=tf.float64)
...
class TRPOAgent(object):
    ...
    def __init__(self, env, observation_dimensions=11, latent_dimensions=3, action_dimensions=3):
        """
    ...
    def run(self, episode_num, vae=False, bcloning=False, fname='0%_validate'):
        if vae:
            ...
            enc_saver.restore(self.sess,"./VAE/results_gumbel_softmax/checkpoint/run8_batch32_correctgraphs/trpo_plugins/encoder/encoder_model_e2000.ckpt")
            saver.restore(self.sess,"./VAE/results_gumbel_softmax/checkpoint/run8_batch32_correctgraphs/trpo_plugins/decoder/decoder_model_e2000.ckpt")
            ...
            with open(out_file, 'w') as results, open(action_out_file, 'w') as action_results:
                ...
                results.write(
                    "episode,var1,var2,var3,var4,var5,var6,var7,var8,var9,var10,var11,latent1,latent2,latent3\n")
            ...
            out_file = myconfig['output_dir']+'/exp'+str(myconfig['exp'])+'_'+fname+'_D-Info_GAIL_results-3modes.csv'
            action_out_file = myconfig['output_dir'] + '/exp'+str(myconfig['exp'])+'_'+fname+'_D-Info_GAIL_actions_results-3modes.csv'
            ...
            with open(out_file, 'w') as results, open(action_out_file, 'w') as action_results:
                #results.write("episode,var1,var2,var3,var4,var5,var6,var7,var8,var9,var10,var11\n")
                results.write("episode,var1,var2,var3,var4,var5,var6,var7,var8,var9,var10,var11,latent1,latent2,latent3\n")
                action_results.write("episode,action1,action2,action3\n")
                actions = []
                obs = []
                discounted_rewards = []
                ...
                for i_episode in range(episode_num):
                    ...
                    line = ""
                    # EDW ALLAXTHKE
                    dec_input1, latent_init, argmax = self.init_encoder(observation)
                    arg1 = argmax
                    # print('observation:', observation)
                    # print('INIT_OBSERVATION:', dec_input1)
                    ...
                    for t in range(self.path_size):
                        ...
                        #time.sleep(0.0001)
                        self.env.render(arg1)
                        #self.env.render(arg1)

                        observation2, reward, done, _ = self.env.step(action)
                        dec_input2, latent, argmax = self.new_encoder(observation2, latent_init)
                        arg1 = argmax
                        # print('observation2:', observation2)
                        # print('_OBSERVATION2:', dec_input2)
                        steps += 1
    ...
    def rollout(self, mini_batch, latent_sequence_np):
        ...
        if mini_batch % 100 == 0:
            np.savetxt(str(myconfig['exp'])+'_'+str(mini_batch)+"_observation_log.csv", np.asarray(log_observations)
                       , delimiter=',', header='var1,var2,var3,var4,var5,var6,var7,var8,var9,var10,var11,latent1,latent2,latent3,episode',
                       comments='')
    ...
    def train(self, expert_observations, expert_actions):
        ...
        encoder_saver.restore(self.sess, "./VAE/results_gumbel_softmax/checkpoint/run8_batch32_correctgraphs/trpo_plugins/encoder/encoder_model_e2000.ckpt")
        saver = tf.train.Saver(self.model.weights)
        #saver.restore(self.sess, "./checkpoint/bcloning.ckpt")
        saver.restore(self.sess, "./VAE/results_gumbel_softmax/checkpoint/run8_batch32_correctgraphs/trpo_plugins/decoder/decoder_model_e2000.ckpt")
        ...
        for mini_batch in range(self.mini_batches+1):
            ...
            observations_np = np.array(flat_observations2, dtype=np.float64) #11
            #print('obs_np:', observations_np[0])
            observations_np2 = np.array(flat_observations, dtype=np.float64) #14
