...
class TRPOAgent(object):
    ...
    def __init__(self, env, observation_dimensions=10, latent_dimensions=5, action_dimensions=3):
        """
    ...
    def run(self, episode_num, vae=False, bcloning=False, fname='0%_validate'):
        if bcloning:
            ...
            out_file = 'VAE/results_gumbel_softmax/checkpoint/run3(batch-32-modes5-nokl)/trpo_plugins/0_VAE_results.csv'
            action_out_file = 'VAE/results_gumbel_softmax/checkpoint/run3(batch-32-modes5-nokl)/trpo_plugins/0_actions_VAE_results.csv'

            enc_saver = tf.train.Saver(self.model2.weights)
            enc_saver.restore(self.sess,"VAE/results_gumbel_softmax/checkpoint/run3(batch-32-modes5-nokl)/trpo_plugins/encoder/encoder_model_e2000.ckpt")  # run2(batch-32-modes3)
            saver = tf.train.Saver(self.model.weights)
            saver.restore(self.sess,"VAE/results_gumbel_softmax/checkpoint/run3(batch-32-modes5-nokl)/trpo_plugins/decoder/decoder_model_e2000.ckpt")  # run2(batch-32-modes3)
            ...
            out_file = myconfig['output_dir']+'/exp'+str(myconfig['exp'])+'_'+fname+'_D-Info_GAIL_results-NOA(5modes).csv'
            action_out_file = myconfig['output_dir'] + '/exp'+str(myconfig['exp'])+'_'+fname+'_D-Info_GAIL_actions_results-NOA(5modes).csv'
        ...
        if mini_batch % 100 == 0:
            np.savetxt(myconfig['exp']+'_'+str(mini_batch)+"_observation_log.csv", np.asarray(log_observations)
                       , delimiter=',', header='longitude,latitude,altitude,timestamp,'+
                                               ...
                                               'v-component_of_wind_isobaric,'+'latent1,latent2,latent3,episode'
                       , comments='')
    ...
    def train(self, expert_observations, expert_actions):
        ...
        encoder_saver.restore(self.sess,"./VAE/results_gumbel_softmax/checkpoint/run3(batch-32-modes5-nokl)/trpo_plugins/encoder/encoder_model_e2000.ckpt")
        saver = tf.train.Saver(self.model.weights)
        # saver.restore(self.sess, "./checkpoint/bcloning.ckpt")
        saver.restore(self.sess, "./VAE/results_gumbel_softmax/checkpoint/run3(batch-32-modes5-nokl)/trpo_plugins/decoder/decoder_model_e2000.ckpt")
        ...
        for mini_batch in range(self.mini_batches+1):
            ...
            flat_observations2 = flat_observations[:, :10]
