...
def discriminator_load_test():
    for files in [['./exp0_0%_test_GAIL_results.csv',#obs,
        ...
        log_obs = pd.read_csv('../../run/Directed-INFO GAIL/run3/0_0_observation_log.csv').drop(['episode', 'latent1', 'latent2', 'latent3'], axis=1)
        log_act = pd.read_csv('../../run/Directed-INFO GAIL/run3/0_0_action_log.csv').drop(['episode'], axis=1)
...
print('actions_dir-info gail:', len(actions_gail), 'obs_dir-info gail:', len(obs_gail), '\n')
