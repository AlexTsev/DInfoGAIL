...
from environment import environment_raw
...
flags.DEFINE_string('checkpoint_dir', os.getcwd() + '/results_gumbel_softmax/checkpoint/' + 'run6-raw(3modes)', 'Directory for checkpoints')#current_time
...
flags.DEFINE_integer('encoder_n_input', 4, 'input size')
...
flags.DEFINE_integer('decoder_n_input', 4, 'input size')
...
def starting_points_2(observations,fname):
    ...
    for i in range(4):
        pd.DataFrame(observations_prcnt[i], columns=['trajectory_ID', 'longitude', 'latitude', 'altitude', 'timestamp']).to_csv(
            myconfig['input_dir'] + '/' + str(int(n[i]*100)) +
            '%_' + fname + '_starting_points.csv',
            index=False,
        )
        pd.DataFrame(trajectories_prcnt[i], columns=['trajectory_ID', 'longitude', 'latitude', 'altitude', 'timestamp']).to_csv(
            myconfig['input_dir'] + '/' + str(int(n[i]*100)) +
...
def encoder(x, latent, temperature):
    ...
    while(i<32):
        ...
        encoder_input_layer = Input(shape=(7,), batch_size=1, tensor=enc_inp, dtype=tf.float64)
...
def decoder(x, latents):
    ...
    #Tensorflow Keras Layers
    decoder_input_layer = Input(shape=(7,), batch_size=32, tensor=dec_inp, dtype=tf.float64)
...
obs_train_df = df_trajectories[['trajectory_ID','longitude','latitude','altitude','timestamp']]

obs_test_df = test_trajectories[['trajectory_ID', 'longitude', 'latitude', 'altitude', 'timestamp']]
...
def train():
    ...
    inputs = tf.placeholder(tf.float64, shape=[None, 4], name='enc_inp')
...
def plot_vae(ploti, latent_var, e):
    ...
    for i in range(0, 327072, 1000):
        ...
        plt.title("Aviation RAW - Barcelona to Madrid")
...
def renameVAE():
    ...
    OLD_CHECKPOINT_FILE = './results_gumbel_softmax/checkpoint/run6-raw(3modes)' + '/encoder/encoder2000/encoder_model_e1999-i327040.ckpt'  # 327040 #'/encoder/encoder_model49599.ckpt'
    NEW_CHECKPOINT_FILE = './results_gumbel_softmax/checkpoint/run6-raw(3modes)' + '/test/encoder/encoder_model_e2000.ckpt'  # '/encoder_new(2)/encoder_model_new49600.ckpt'
    ...
    OLD_CHECKPOINT_FILE2 = './results_gumbel_softmax/checkpoint/run6-raw(3modes)' + '/decoder/decoder2000/decoder_model_e1999-i327040.ckpt'  # 1999-49999.ckpt'
    NEW_CHECKPOINT_FILE2 = './results_gumbel_softmax/checkpoint/run6-raw(3modes)' + '/test/decoder/decoder_model_e2000.ckpt'  # 2000_new50000.ckpt'
...
def rename():
    ...
    # RENAME MAP LAYERS-BIASES-GRAPHS
    OLD_CHECKPOINT_FILE = './results_gumbel_softmax/checkpoint/run6-raw(3modes)' + '/encoder/encoder2000/encoder_model_e1999-i327040.ckpt'  # '/encoder/encoder_model49599.ckpt'
    NEW_CHECKPOINT_FILE = './results_gumbel_softmax/checkpoint/run6-raw(3modes)' + '/trpo_plugins/encoder/encoder_model_e2000.ckpt'  # '/encoder_new(2)/encoder_model_new49600.ckpt'
    # OLD_CHECKPOINT_FILE = FLAGS.checkpoint_dir + '/model9999.ckpt'
    # NEW_CHECKPOINT_FILE = FLAGS.checkpoint_dir + '/model_new/model_new9999.ckpt'
    ...
    OLD_CHECKPOINT_FILE2 = './results_gumbel_softmax/checkpoint/run6-raw(3modes)' + '/decoder/decoder2000/decoder_model_e1999-i327040.ckpt'  # 1999-49999.ckpt'
    NEW_CHECKPOINT_FILE2 = './results_gumbel_softmax/checkpoint/run6-raw(3modes)' + '/trpo_plugins/decoder/decoder_model_e2000.ckpt'  # 2000_new50000.ckpt'
...
class Agent(object):
    def __init__(self, observation_dimensions=4, latent_dimensions=3, action_dimensions=3):#latent_dimensions=5
        ...
        self.env = environment_raw.Environment()
    ...
    def test(self):
        out_file = 'results_gumbel_softmax/checkpoint/run6-raw(3modes)/test/0_VAE_results.csv'
        action_out_file = 'results_gumbel_softmax/checkpoint/run6-raw(3modes)/test/0_actions_VAE_results.csv'

        enc_saver = tf.train.Saver(self.model2.weights)
        enc_saver.restore(self.sess, "results_gumbel_softmax/checkpoint/run6-raw(3modes)/test/encoder/encoder_model_e2000.ckpt")#run2(batch-32-modes3)
        saver = tf.train.Saver(self.model.weights)
        saver.restore(self.sess, "results_gumbel_softmax/checkpoint/run6-raw(3modes)/test/decoder/decoder_model_e2000.ckpt")#run2(batch-32-modes3)
