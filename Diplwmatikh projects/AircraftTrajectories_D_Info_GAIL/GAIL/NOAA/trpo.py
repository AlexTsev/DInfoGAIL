...
class TRPOAgent(object):
    ...
    def run(self, episode_num, bcloning=False, fname='0%_validate'):
        ...
        with open(out_file, 'w') as results, open(action_out_file, 'w') as action_results:
            ...
            for i_episode in range(episode_num):
                r_i = []
                observation, norm_observation = self.env.reset()
                line = ""
                ...
                if bcloning:
                    saver.restore(self.sess,
                                  myconfig['output_dir'] + "/bcloning/bcloning.ckpt")
                else:
                ...
                for t in range(self.path_size):
                    ...
                    action = self.sess.run(self.logits, feed_dict={self.x: [norm_observation]})[0]
                    observation, norm_observation, reward, done = self.env.step(action, t)
                    r_i.append(reward)
    ...
    def train(self, expert_observations, expert_actions):
        ...
        saver = tf.train.Saver(self.model.weights)
        saver.restore(self.sess, myconfig['output_dir'] + "GAIL.ckpt")
