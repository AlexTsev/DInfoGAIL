...
class TRPOAgent(object):
    ...
    def run(self, episode_num, vae=False, bcloning=False, fname='0%_validate'):
        if bcloning:
            ...
            out_file = './VAE/results_gumbel_softmax/checkpoint/run6-raw(3modes)/trpo_plugins/0_VAE_results.csv'
            action_out_file = './VAE/results_gumbel_softmax/checkpoint/run6-raw(3modes)/trpo_plugins/0_actions_VAE_results.csv'

            enc_saver = tf.train.Saver(self.model2.weights)
            enc_saver.restore(self.sess,"VAE/results_gumbel_softmax/checkpoint/run6-raw(3modes)/trpo_plugins/encoder/encoder_model_e2000.ckpt")  # run2(batch-32-modes3)
            saver = tf.train.Saver(self.model.weights)
            saver.restore(self.sess,"VAE/results_gumbel_softmax/checkpoint/run6-raw(3modes)/trpo_plugins/decoder/decoder_model_e2000.ckpt")  # run2(batch-32-modes3)
            ...
            out_file = myconfig['output_dir']+'/exp'+str(myconfig['exp'])+'_'+fname+'_D-INFO_GAIL_results(RAW).csv'
            action_out_file = myconfig['output_dir'] + '/exp'+str(myconfig['exp'])+'_'+fname+'_D-INFO_GAIL_actions_results(RAW).csv'
    ...
    def train(self, expert_observations, expert_actions):
        ...
        encoder_saver.restore(self.sess,"./VAE/results_gumbel_softmax/checkpoint/run6-raw(3modes)/trpo_plugins/encoder/encoder_model_e2000.ckpt")
        saver = tf.train.Saver(self.model.weights)
        # saver.restore(self.sess, "./checkpoint/bcloning.ckpt")
        saver.restore(self.sess, "./VAE/results_gumbel_softmax/checkpoint/run6-raw(3modes)/trpo_plugins/decoder/decoder_model_e2000.ckpt")
