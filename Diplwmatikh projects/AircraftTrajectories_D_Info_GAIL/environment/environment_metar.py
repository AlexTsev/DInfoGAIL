...
class Environment(object):
    ...
    def reset(self):
        ...
        if not self.random_choice:
            ...
            self.metar_state = self.state[10:15]
        else:
            self.state = copy.deepcopy(random.choice(self.starting_points))
            self.metar_state = self.state[10:15]

        #print('INITIAL: ', self.metar_state)
        ...
        self.norm_state = [(self.state[0]-myconfig['longitude_avg'])/myconfig['longitude_std'],
                           ...
                           (self.state[10] - myconfig['drct_avg']) / myconfig['drct_std'],
                           (self.state[11] - myconfig['sknt_avg']) / myconfig['sknt_std'],
                           (self.state[12] - myconfig['alti_avg']) / myconfig['alti_std'],
                           (self.state[13] - myconfig['vsby_avg']) / myconfig['vsby_std'],
                           (self.state[14] - myconfig['gust_avg']) / myconfig['gust_std'],

                           ]
        self.norm_metar_state = self.norm_state[10:15]
        #print('INITIAL_norm: ', self.norm_metar_state)
    ...
    def step(self, action, tstep):
        ...
        #print('test \n:', self.metar_state)
        self.state.extend(self.metar_state)
        ...
        #print('norm_st:', self.norm_state[10:15])
        reward = 0
