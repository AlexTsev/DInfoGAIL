...
flags.DEFINE_string('checkpoint_dir', os.getcwd() + '/results_gumbel_softmax/checkpoint/' + 'run7-enriched_Metar(3modes)', 'Directory for checkpoints')#current_time
...
def encoder(x, latent, temperature):
    ...
    while(i<32):
        ...
        encoder_input_layer = Input(shape=(18,), batch_size=1, tensor=enc_inp, dtype=tf.float64)
...
def decoder(x, latents):
    ...
    #Tensorflow Keras Layers
    decoder_input_layer = Input(shape=(18,), batch_size=32, tensor=dec_inp, dtype=tf.float64)
...
def renameVAE():
    ...
    OLD_CHECKPOINT_FILE = './results_gumbel_softmax/checkpoint/run7-enriched_Metar(3modes)' + '/encoder/encoder2000/encoder_model_e1999-i327040.ckpt'  # 327040 #'/encoder/encoder_model49599.ckpt'
    NEW_CHECKPOINT_FILE = './results_gumbel_softmax/checkpoint/run7-enriched_Metar(3modes)' + '/test/encoder/encoder_model_e2000.ckpt'  # '/encoder_new(2)/encoder_model_new49600.ckpt'
    ...
    OLD_CHECKPOINT_FILE2 = './results_gumbel_softmax/checkpoint/run7-enriched_Metar(3modes)' + '/decoder/decoder2000/decoder_model_e1999-i327040.ckpt'  # 1999-49999.ckpt'
    NEW_CHECKPOINT_FILE2 = './results_gumbel_softmax/checkpoint/run7-enriched_Metar(3modes)' + '/test/decoder/decoder_model_e2000.ckpt'  # 2000_new50000.ckpt'
...
def rename():
    ...
    # RENAME MAP LAYERS-BIASES-GRAPHS
    OLD_CHECKPOINT_FILE = './results_gumbel_softmax/checkpoint/run7-enriched_Metar(3modes)' + '/encoder/encoder2000/encoder_model_e1999-i327040.ckpt'  # '/encoder/encoder_model49599.ckpt'
    NEW_CHECKPOINT_FILE = './results_gumbel_softmax/checkpoint/run7-enriched_Metar(3modes)' + '/trpo_plugins/encoder/encoder_model_e2000.ckpt'  # '/encoder_new(2)/encoder_model_new49600.ckpt'
    # OLD_CHECKPOINT_FILE = FLAGS.checkpoint_dir + '/model9999.ckpt'
    # NEW_CHECKPOINT_FILE = FLAGS.checkpoint_dir + '/model_new/model_new9999.ckpt'
    ...
    OLD_CHECKPOINT_FILE2 = './results_gumbel_softmax/checkpoint/run7-enriched_Metar(3modes)' + '/decoder/decoder2000/decoder_model_e1999-i327040.ckpt'  # 1999-49999.ckpt'
    NEW_CHECKPOINT_FILE2 = './results_gumbel_softmax/checkpoint/run7-enriched_Metar(3modes)' + '/trpo_plugins/decoder/decoder_model_e2000.ckpt'  # 2000_new50000.ckpt'
...
class Agent(object):
    def __init__(self, observation_dimensions=15, latent_dimensions=3, action_dimensions=3):#latent_dimensions=5
    ...
    def one_hot_encoding(self, x):
        ...
        encoded = to_categorical(argmax, num_classes=3)#3)
        # print('enc:', encoded)
    ...
    def test(self):
        out_file = 'results_gumbel_softmax/checkpoint/run7-enriched_Metar(3modes)/test/0_VAE_results_metar.csv'
        action_out_file = 'results_gumbel_softmax/checkpoint/run7-enriched_Metar(3modes)/test/0_actions_VAE_results_metar.csv'

        enc_saver = tf.train.Saver(self.model2.weights)
        enc_saver.restore(self.sess, "results_gumbel_softmax/checkpoint/run7-enriched_Metar(3modes)/test/encoder/encoder_model_e2000.ckpt")#run2(batch-32-modes3)
        saver = tf.train.Saver(self.model.weights)
        saver.restore(self.sess, "results_gumbel_softmax/checkpoint/run7-enriched_Metar(3modes)/test/decoder/decoder_model_e2000.ckpt")#run2(batch-32-modes3)
