...
from Dir_Info_GAIL.METAR import trpo_m_5
import critic
...
def bcloning(x_train, y_train, x_test, y_test):
    ...
    checkpoint_path = myconfig['output_dir']+"/bcloning.ckpt"
...
def plot_policy():
    model, input = trpo_m_5.Policy.create_policy(action_dimensions=3,
...
def plot_discriminator():
    discriminator = trpo_m_5.Discriminator(action_dimensions=3,
                                           observation_dimensions=3)
...
if args.split_dataset:
    ...
    train_df = pd.read_csv(myconfig['input_dir'] + 'dataset/train_validation_set.csv')

    obs_train_df = train_df[['trajectory_ID','longitude','latitude','altitude','timestamp','Pressure_surface',
                                         'Relative_humidity_isobaric','Temperature_isobaric','Wind_speed_gust_surface',
                                         'u-component_of_wind_isobaric','v-component_of_wind_isobaric']]
    obs_train_df.groupby('trajectory_ID').\
        head(1).to_csv(myconfig['input_dir']+'dataset/train_starting_points.csv', index=False)
...
agent = trpo_m_5.TRPOAgent(env, action_dimensions=3, observation_dimensions=10)
...
for prcnt in [0,20,50,70]:
    ...
    actions_bcloning, obs_bcloning, discounted_rewards_bcloning, total_rewards_bcloning = agent.run(test_flight_num, bcloning=True, fname=fname)

    # env.read_starting_points(random_choice=True,fname='train')

    # env.read_starting_points(random_choice=False, fname=fname)
    actions_gail, obs_gail, discounted_rewards_gail, total_rewards_gail = agent.run(test_flight_num, bcloning=False, fname=fname)
