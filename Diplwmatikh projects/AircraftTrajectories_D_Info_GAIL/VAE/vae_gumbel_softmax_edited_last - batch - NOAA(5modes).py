...
flags.DEFINE_string('checkpoint_dir', os.getcwd() + '/results_gumbel_softmax/checkpoint/' + 'run3(batch-32-modes5-nokl)', 'Directory for checkpoints')#current_time
...
#lags.DEFINE_integer('latent_nz', 3, 'latent size')
...
def decoder(x, latents):
    ...
    #Tensorflow Keras Layers
    decoder_input_layer = Input(shape=(15,), batch_size=32, tensor=dec_inp, dtype=tf.float64)
...
train_starting_points = pd.read_csv('../dataset/NOAA/train_starting_points.csv')
...
def plot_vae(ploti, latent_var, e):
    ...
    for i in range(0, 327072, 1000):
        ...
        plt.title("Aviation NOAA - Barcelona to Madrid")
...
def renameVAE():
    ...
    OLD_CHECKPOINT_FILE = './results_gumbel_softmax/checkpoint/run3(batch-32-modes5-nokl)' + '/encoder/encoder2000/encoder_model_e1999-i327040.ckpt'  # 327040 #'/encoder/encoder_model49599.ckpt'
    NEW_CHECKPOINT_FILE = './results_gumbel_softmax/checkpoint/run3(batch-32-modes5-nokl)' + '/test/encoder/encoder_model_e2000.ckpt'  # '/encoder_new(2)/encoder_model_new49600.ckpt'
    ...
    OLD_CHECKPOINT_FILE2 = './results_gumbel_softmax/checkpoint/run3(batch-32-modes5-nokl)' + '/decoder/decoder2000/decoder_model_e1999-i327040.ckpt'  # 1999-49999.ckpt'
    NEW_CHECKPOINT_FILE2 = './results_gumbel_softmax/checkpoint/run3(batch-32-modes5-nokl)' + '/test/decoder/decoder_model_e2000.ckpt'  # 2000_new50000.ckpt'
...
def rename():
    ...
    # RENAME MAP LAYERS-BIASES-GRAPHS
    OLD_CHECKPOINT_FILE = './results_gumbel_softmax/checkpoint/run3(batch-32-modes5-nokl)' + '/encoder/encoder2000/encoder_model_e1999-i327040.ckpt'  # '/encoder/encoder_model49599.ckpt'
    NEW_CHECKPOINT_FILE = './results_gumbel_softmax/checkpoint/run3(batch-32-modes5-nokl)' + '/trpo_plugins/encoder/encoder_model_e2000.ckpt'  # '/encoder_new(2)/encoder_model_new49600.ckpt'
    # OLD_CHECKPOINT_FILE = FLAGS.checkpoint_dir + '/model9999.ckpt'
    # NEW_CHECKPOINT_FILE = FLAGS.checkpoint_dir + '/model_new/model_new9999.ckpt'
    ...
    OLD_CHECKPOINT_FILE2 = './results_gumbel_softmax/checkpoint/run3(batch-32-modes5-nokl)' + '/decoder/decoder2000/decoder_model_e1999-i327040.ckpt'  # 1999-49999.ckpt'
    NEW_CHECKPOINT_FILE2 = './results_gumbel_softmax/checkpoint/run3(batch-32-modes5-nokl)' + '/trpo_plugins/decoder/decoder_model_e2000.ckpt'  # 2000_new50000.ckpt'
...
class Agent(object):
    def __init__(self, observation_dimensions=10, latent_dimensions=5, action_dimensions=3):#latent_dimensions=5
